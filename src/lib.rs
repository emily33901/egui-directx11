            ...
            match primitive {
                ...
                Primitive::Callback(callback) => {
                    ...
                    let viewport = D3D11_VIEWPORT {
                        TopLeftX: viewport_px.left_px as f32,
                        TopLeftY: viewport_px.top_px as f32,
                        Width: viewport_px.width_px as f32,
                        Height: viewport_px.height_px as f32,
                        MinDepth: 0.0,
                        MaxDepth: 1.0,
                    };
                    ...
                    if let Some(callback) = callback.callback.downcast_ref::<CallbackFn>() {
                        ...
                        log::warn!(
                            "Warning: Unsupported render callback. Expected egui_directx11::CallbackFn"
                        );
                    }
...
pub struct CallbackFn {
    f: Box<dyn Fn(egui::PaintCallbackInfo, &Renderer) + Sync + Send>,
}

impl CallbackFn {
    pub fn new<F: Fn(egui::PaintCallbackInfo, &Renderer) + Sync + Send + 'static>(
        callback: F,
    ) -> Self {
